package org.coursera.epl.functional.design.week2.pouring
...
class Pouring(capacity: Vector[Int]) {
  ...
  case class Pour(from: Int, to: Int) extends Move {
    override def change(state: State): State = {
      val amount = state(from) min (capacity(to) - state(from))
  ...
  class Path(moves: List[Move], val endState: State) {
    def extend(move: Move) = new Path(move :: moves, move change endState)

    override def toString: String = (moves.reverse mkString " ") + "--> " + endState
  }
  ...
  val pathsSet: Stream[Set[Path]] = from(Set(initialPath), Set(initialState))
